...
- **API** : Hono on Node 24, Drizzle on Postgres 16 (PostGIS), OVH S3-compatible object storage in deployed environments (MinIO locally), `@sentropic/llm-mesh` for multi-provider LLM access, `@sentropic/chat-core` for orchestration, `graphifyy` for the knowledge graph linking documents.
...
- **Deployment** : SPA on GitHub Pages, server on OVH Managed Kubernetes, and dedicated OVH S3 buckets. Domain `immo.sent-tech.ca`.
