...
filter="$root/deploy/ci/docs-prod-parity-receipt.jq"
digest="$(printf 'a%.0s' {1..64})"
receipt="$(jq -n --arg digest "$digest" '
  {processed:59017,logicalBytes:12534514457,failed:0,canonicalDigest:$digest,
...
[ "$(grep -c 'docs-prod-parity-receipt.jq' "$root/deploy/ci/object-storage-prod.mk")" -eq 3 ]
