# Astra low avec repli Gemini low : acceptation et promotion

Refs #703 et #697. Décision finale de l'owner du 2026-09-17 : la politique est `openai / gpt-6-astra / low` en principal (100/100 documents acceptés au benchmark, F1 net 0,632), avec repli `gemini / gemini-3.8-flash / low` sur incident transport, quota, délai, flux vide ou refus qualité persistant. `REFRESH_PRIMARY_QUALITY_ATTEMPTS=2` reste en place mais ne se déclenche pas : Astra low n'a produit aucun refus qualité sur le corpus. La cascade Gemini d'abord reste documentée comme option économique écartée. Un refus du contrat v9 (profil ou provenance) est rejoué une fois sur le modèle principal, puis basculé sur Gemini ; le refus persistant bascule vers `openai / gpt-6-astra / low`. Transport, quota/429, délai ou flux vide basculent immédiatement vers Astra. `REFRESH_FORCE_FALLBACK=1` reste une recette contrôlée et contourne Gemini.
...
3. Archiver les reçus sûrs et mesurer : modèle par document, nombre de tentatives, transitions, taux de récupération du modèle principal (acceptés après rejeu / refus initiaux), bascules vers Gemini, quota/429 et signaux B′ publiés.
...
5. Exécuter séparément une recette `REFRESH_FORCE_FALLBACK=1`; le reçu doit montrer Gemini low, `transition=fallback`, `fallbackReason=forced` et aucun appel Astra. Cette recette ne remplace pas le cycle normal.
