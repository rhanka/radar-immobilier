...
**Latest continuation boundary:** the current design uses **Graphify with in-process mesh inside the Immo pod**, superseding the earlier hybrid/network-service assumption. Immo retains corpus/checkpoints/gates/canonical publication; Graphify supplies its library/provider lifecycle. Graphify 0.18.0 is published; by the 15:39 UTC progress capture Immo had pinned the refresh dependencies at `d0595d9f`, passed typecheck and started scoped tests. Consumer completion, operated keyring/lock and end-to-end acceptance remain open. This is implementation progress, not deployed runtime. The [D5 target dossier](architecture/decision-dossier.md) and [sequential states](architecture/transitions-target.md) place architecture before billing. **SCW TEM remains until its replacement is validated.**
...
| Immo refresh continuation | `d0595d9f` (`feat/refresh-018`) plus `docs/reviews/refresh-018/preprod-readiness.md`, captured 15:38–15:39 UTC | Dependency/typecheck progress and dedicated preprod workload/CronJob read; no Graphify 0.18 runtime, typed Signal/PDF or production claim |
