...
1. **Réconcilier live→git** : capturer la ResourceQuota préprod live en manifeste canonique (candidats à confirmer de mon read : `deploy/k8s/00-namespace.yaml`, `deploy/overlays/preprod/kustomization.yaml`, `deploy/ci/reconcile-preprod.sh`), puis CD = source de vérité.
