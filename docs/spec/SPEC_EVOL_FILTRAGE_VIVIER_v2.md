...
- **Deltas 3.4** : reclasser les **1533 `category=null`** ; poser `instrument`, `usage_dominant` ; `effet_densifiant` reste ABSENT (== inconnu via défaut Vivier ; pas de placeholder écrit), rempli par geo ensuite, **étape propre + historique**, scanner **PV-complet (#368)** (récupère points secondaires/annexes → anti-silence).
