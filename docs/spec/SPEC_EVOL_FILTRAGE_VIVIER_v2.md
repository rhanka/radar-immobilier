...
- **Numérotation (à réconcilier)** : le code utilise `ontology_version` **2.0/2.1/2.2/2.3** ; le vocabulaire « 3.x » de la spec désigne les **passes d'enrichissement cumulatives** (3.1↔v2.1 `etape` ; 3.2↔v2.2 `zone_ref/no_lot/reglement_number` ; 3.3↔v2.3 évidence). **3.4 doit trancher** : soit renommer la ligne d'ontologie en 3.x, soit assumer explicitement le mapping. Décalage réel, non résolu à ce jour.
