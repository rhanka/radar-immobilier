...
const checkExpression = `(() => {
  ...
  if (!document.querySelector('.masthead').textContent.includes('3 SCÈNES · 12 SECTIONS')) throw Error('bandeau des trois scènes absent');
  const metrics = [];
...
// Les choix de §12 : D1 à D5, une seule réponse chacun, réellement sélectionnables.
const picks = { D1: 'A', D2: 'CONDUCTEUR', D3: 'MESURER', D4: 'AUTRE_REGION', D5: 'BUCKET' };
await evaluate(`(() => {
  document.querySelector('[data-question="D1"] [data-option="C"] input').click();
  for (const [question, option] of Object.entries(${JSON.stringify(picks)}))
    document.querySelector('[data-question="' + question + '"] [data-option="' + option + '"] input').click();
  return true;
})()`);
await pause(400);
const choices = await evaluate(`(() => {
  ...
  const selected = Object.fromEntries(blocks.map(block => [block.dataset.question,
    [...block.querySelectorAll('[data-option][data-selected="true"]')].map(option => option.dataset.option)]));
  ...
  const persisted = JSON.parse(localStorage.getItem(Object.keys(localStorage).find(key => key.startsWith('immo-712-v2-decision-responses:'))) ?? '{}').selections ?? {};
  return { blocks: blocks.length, single: single.length, radio, selected, answered,
    selections: Object.fromEntries(pack.responses.map(response => [response.key, response.selection])),
    optionKeys: Object.fromEntries(pack.responses.map(response => [response.key, response.options.map(option => option.key)])),
...
if (choices.blocks !== 5 || choices.single !== 5 || choices.radio !== 16 || choices.options !== 16
  || JSON.stringify(choices.selections) !== JSON.stringify(picks)
  || JSON.stringify(choices.persisted) !== JSON.stringify(picks)
  || Object.entries(picks).some(([question, option]) => JSON.stringify(choices.selected[question]) !== JSON.stringify([option]))
  || choices.answered.length !== 5 || choices.status !== 'draft-not-ratified'
  || choices.schema !== 'immo-712-decision-owner-response/v2')
