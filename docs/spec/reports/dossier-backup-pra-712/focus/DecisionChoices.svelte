<script>
  ...
  // (brouillon local, export JSON réel) : les cinq décisions D1 à D5 de §12.
  ...
  let storageKey = $derived(`immo-712-v2-decision-responses:${manifest.artifactInputHash}`);
  ...
  function download() {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'reponses-dossier-712-backup-pra-v2.json'; a.click(); URL.revokeObjectURL(url);
  }
</script>

<section class="choices" id="ce-qu-on-demande" aria-labelledby="choice-title">
  <div class="flex-row">
    <div><span class="eyebrow">§12 · ce que j’attends de toi</span><h2 id="choice-title">Choix sélectionnables</h2></div>
    <span class="badge warning">Brouillon local · rien n’est ratifié</span>
  </div>
  <p>Les cinq décisions de §12, dans l’ordre du dossier, chacune avec ses options. Les réponses restent locales : elles ne
    fusionnent aucune PR, ne déploient rien, ne déclenchent aucune action cluster ni aucun acte de production.</p>
  ...
  <details class="choice-json" open={Boolean(copyError)}>
    ...
    <textarea aria-label="JSON des réponses au dossier Sauvegardes et PRA v2 (PR #712 et geo#390)" readonly value={json} rows={18}></textarea>
  </details>
</section>
