<script>
  ...
  let storageKey = $derived(`immo-kanban-decision-responses:${manifest.artifactInputHash}`);
  ...
  function download() {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'reponses-kanban-iteration.json'; a.click(); URL.revokeObjectURL(url);
  }
</script>
...
<section class="choices" id="ce-qu-on-demande" aria-labelledby="choice-title">
  ...
  <p>Chaque question précède ses options, dans l’ordre de §7. Les réponses restent locales : elles ne créent
    aucune issue, aucun projet GitHub, aucun événement track et ne déclenchent aucun déploiement.</p>
  ...
  <details class="choice-json" open={Boolean(copyError)}>
    ...
    <textarea aria-label="JSON des réponses au dossier Kanban" readonly value={json} rows={18}></textarea>
  </details>
</section>
