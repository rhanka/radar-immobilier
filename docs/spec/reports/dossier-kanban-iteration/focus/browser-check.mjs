...
const port = process.env.CDP_PORT ?? '9239';
...
// Les choix de §7 : réellement sélectionnables, en une seule et en plusieurs réponses.
await evaluate(`(() => {
  document.querySelector('[data-question="q1-way-of-working"] [data-option="A"] input').click();
  const items = document.querySelector('[data-question="q2-items"]');
  items.querySelector('[data-option="O2"] input').click();
  items.querySelector('[data-option="T12"] input').click();
  document.querySelector('[data-question="q3-creation"] [data-option="GO"] input').click();
  return true;
})()`);
await pause(400);
const choices = await evaluate(`(() => {
  ...
  const multi = blocks.filter(block => block.dataset.mode === 'multi');
  const radio = document.querySelectorAll('.question-block[data-mode="single"] input[type="radio"]').length;
  const checkbox = document.querySelectorAll('.question-block[data-mode="multi"] input[type="checkbox"]').length;
  const q1Selected = document.querySelector('[data-question="q1-way-of-working"] [data-option="A"]').dataset.selected === 'true';
  const items = document.querySelector('[data-question="q2-items"]');
  const multiSelected = [...items.querySelectorAll('[data-option][data-selected="true"]')].map(option => option.dataset.option);
  ...
  const q2 = pack.responses.find(response => response.key === 'q2-items');
  const persisted = Object.keys(JSON.parse(localStorage.getItem(Object.keys(localStorage).find(key => key.startsWith('immo-kanban-decision-responses:'))) ?? '{}').selections ?? {});
  return { blocks: blocks.length, single: single.length, multi: multi.length, radio, checkbox,
    q1Selected, multiSelected, answered, q2Selection: q2.selection, schema: pack.schema, status: pack.status,
    persisted, options: pack.responses.reduce((total, response) => total + response.options.length, 0) };
})()`);
if (!choices.q1Selected || choices.multiSelected.length !== 2 || choices.multi < 1 || choices.radio < 4
  || choices.answered.length !== 3 || !choices.persisted.includes('q2-items'))
...
console.log(JSON.stringify({ status: report.status, scenes: report.scenes.map(scene => ({ id: scene.sceneId, cards: scene.cards, edges: scene.edges, groups: scene.groups, labels: scene.edgeLabels, fitView: Number(scene.initialScale.toFixed(4)) })), choices: { blocks: choices.blocks, radio: choices.radio, checkbox: choices.checkbox, answered: choices.answered.length }, captures: report.captures.length, consoleErrors: consoleErrors.length, runtimeErrors: runtimeErrors.length, externalRequests: externalRequests.length }));
ws.close();
