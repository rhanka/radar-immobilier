...
## 2. Champs v2.2 — définitions et règles d'extraction
...
**OBJET : RE-GRAPHIFY v2.2 — extraction structurée zone_ref / no_lot / reglement_number (villes M–Z)**
...
**Ontologie de référence :** `radar/ontology/ontology-profile.yaml` v2.2 (branche
`feat/ontology-v2.2-extraction`). Tu dois lire ce profil avant de lancer graphify.
La `ontology_version` de sortie doit être `"2.2"`.

**Contrat de sortie :** `radar/ontology/graphify-output-contract.md` §9 (champs v2.2).
...
-- (à lancer après le re-run du mapper une fois les graphes v2.2 projetés)
SELECT
...
### 7.1 Situation actuelle (avant v2.2)
...
### 7.2 Après v2.2 (honnêteté sur l'estimation)
...
**Taux de résolution mapper estimé après v2.2 :**
...
**Conclusion :** v2.2 permettra au mapper de passer de ~30 % à ~70–85 % de résolution
...
- Profil ontologie v2.2 : `radar/ontology/ontology-profile.yaml`
- Contrat de sortie v2.2 : `radar/ontology/graphify-output-contract.md` §9
