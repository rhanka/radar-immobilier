...
for (const match of markdown.matchAll(/```mermaid\n([\s\S]*?)\n```/g)) {
  ...
  sections.push({ kind: 'diagram', id: `diagram-${sections.length}`, targetRef: 'docs/architecture.md', syntax: 'mermaid', source: match[1], alt: `Architecture diagram ${sections.filter(s => s.kind === 'diagram').length + 1}` });
