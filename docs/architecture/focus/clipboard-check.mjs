...
try {
  ...
  console.log(await evaluate(`(async () => {
    ...
    try {
      ...
      if (pack.revision !== 'D6' || pack.option !== null || pack.note !== note.value || pack.fixedInstructions.reporting.startInclusive !== '2026-08-10T00:00:00-04:00' || pack.fixedInstructions.reporting.hours !== 840 || pack.fixedInstructions.transitionEvidence.t1.firstKubernetesRun !== 'failed-before-LLM' || pack.fixedInstructions.transitionEvidence.t2.rawOvhRebind !== true || pack.fixedInstructions.transitionEvidence.t2.docsInventory.productionScwDocsPocs.objects !== 59017 || pack.fixedInstructions.transitionEvidence.t2.docsCopy !== false || pack.fixedInstructions.transitionEvidence.t3.status !== 'not-started' || pack.fixedInstructions.billing.node.projectedAmountCad !== 68.88 || pack.fixedInstructions.billing.llm.totalFacturableCad !== 251.21543767641742 || 'decision' in pack || 'options' in pack) throw Error('Real clipboard content mismatch');
      document.querySelector('.choices').scrollIntoView({ behavior: 'instant' });
      return { actualClipboard: 'pass', revision: pack.revision, option: pack.option, comment: 'exact', currentAmount: pack.fixedInstructions.billing.node.projectedAmountCad, status: pack.status };
