...
console.log(await evaluate(`(async () => {
  ...
  if (preview.fixedInstructions.reporting.captureCutoffs.architectureRuntime !== '2026-09-13T15:38:00Z' || preview.fixedInstructions.reporting.captureCutoffs.transitionImplementation !== '2026-09-13T15:39:00Z' || preview.fixedInstructions.reporting.captureCutoffs.latestTransitionAudit !== null || preview.fixedInstructions.reporting.captureCutoffs.unifiedDeliveryTokenBilling !== null) throw Error('JSON instructions conflated evidence and unified cutoffs');
  if (preview.fixedInstructions.transitionEvidence.t1.status !== 'BLOCKED' || preview.fixedInstructions.transitionEvidence.extractionBenchmark.scores !== null || preview.fixedInstructions.transitionEvidence.extractionBenchmark.cloudCodeEnrolled !== false || preview.fixedInstructions.transitionEvidence.t2.copyStarted !== false) throw Error('Transition gates lost from JSON instructions');
