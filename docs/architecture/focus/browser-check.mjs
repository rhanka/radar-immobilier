...
const waitUntil = async (expression, failure) => {
  ...
  throw Error(`${failure}: ${JSON.stringify(errors)}`);
};
...
console.log(await evaluate(`(async () => {
  ...
  if (!document.querySelector('.masthead').textContent.includes('TRANSITION EFFECTIVE')) throw Error('D6 three-state masthead missing');
  if (document.querySelector('.flow').dataset.graph !== 'target-1') throw Error('Effective transition is not the default');
  const path = [...document.querySelectorAll('.journey button')].map(button => button.textContent.trim());
  if (path.join('|') !== '0Avant|1Transition effective|2Après T2|3Après cible') throw Error('Three-state path missing: ' + path);
  ...
  if (!instructions.textContent.includes('Aucun choix DIRECT / USAGE / CAPACITY') || !instructions.textContent.includes('0,082 CAD/h') || !instructions.textContent.includes('251,215438 CAD')) throw Error('Fixed billing instructions missing');
  if (document.querySelectorAll('input[type="radio"]').length) throw Error('D6 must not force an allocation choice');
  ...
  if (preview.revision !== 'D6' || preview.option !== null || preview.note !== comment.value || preview.fixedInstructions.billing.allocationMethodChoiceRequired !== false) throw Error('JSON instructions lost provenance/comment');
  if (preview.fixedInstructions.reporting.startInclusive !== '2026-08-10T00:00:00-04:00' || preview.fixedInstructions.reporting.hours !== 840) throw Error('JSON instructions lost report boundary');
  if (preview.fixedInstructions.transitionEvidence.t1.firstKubernetesRun !== 'failed-before-LLM' || preview.fixedInstructions.transitionEvidence.t2.docsCopy !== false || preview.fixedInstructions.transitionEvidence.t2.docsInventory.productionScwDocsPocs.objects !== 59017) throw Error('Transition gates lost from JSON instructions');
  if (preview.fixedInstructions.transitionEvidence.t1.graphify !== '0.18.0' || preview.fixedInstructions.transitionEvidence.t2.decision !== 'MIGRATE+RETAIN' || preview.fixedInstructions.transitionEvidence.t3.verdict !== 'NO-GO today') throw Error('JSON instructions lost transition verdicts');
  if (preview.fixedInstructions.billing.node.periodHours !== 840 || preview.fixedInstructions.billing.node.projectedAmountCad !== 68.88 || 'decision' in preview || 'options' in preview || 'finalBillableCad' in preview) throw Error('JSON instructions lost calculation or invented a vote');
...
await waitUntil(`document.querySelector('.flow')?.dataset.graph === 'target-1' && document.body.textContent.includes('TRANSITION EFFECTIVE')`, 'Dated monthly D6 Focus rendering missing');
