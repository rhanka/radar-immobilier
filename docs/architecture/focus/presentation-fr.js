...
Graphify reste épinglé à **exactement 0.18.0**. Fable a rendu **BLOCK** à **ac3a7150**; les correctifs **537b9e0c + 3d9ed43c** passent scoped **34/34**, intégration **4/4** et typecheck, mais la re-review reste en cours. Conserver llm-mesh **0.19.0** : le rare **UND_ERR_SOCKET** fait échouer le Job fail-closed, puis le cycle suivant reprend l’état durable, sans corruption ni retry in-process revendiqué. 0.19.1 n’est pas un gate; le diagnostic est déféré à **s-conductor**, sans demande d’implémentation. T1 reste **BLOCKED**, sans Signal fournisseur réel ni acceptation Kubernetes.

Avant toute extraction réelle, le benchmark compare sur les **mêmes cinq PDF** la baseline historique/manuelle et les contrats **v1/v2/v3**, avec runs non simulés et traçables. Le modèle n’est pas choisi, **Cloud Code n’est pas enrôlé** et aucun score n’est revendiqué. Ensuite seulement : Signal/PDF réel, rejeu idempotent et Job CronJob après remplacement de pod/credentials. [Pipeline causal](proposal.md).`,
...
La remédiation atteint **ee84ae29 / f2ac3825 / c30467ca / cef6d7ed**. Le mécanisme de checkpoint **aaf0cbf7 / 91242223** reste en construction : ces commits ne sont ni une acceptation ni une copie d’objets. Aucun copy n’a commencé. Après bascule, l’ancien store reste en reprise lecture seule jusqu’au gate de suppression; MinIO ne part qu’à zéro consommateur.
...
`[GATES] T1 : re-review Fable → benchmark cinq PDF non simulé → vrai Signal/PDF → schedule autonome. T2 : matrice clients → checkpoint/writer safety → copie/parité → fence → repoint → restauration → révocation/suppression. T3 : T2 complet → rightsizing → contraintes affinity/PVC → deux nœuds vérifiés → un nœud préprod puis prod.
...
Les limites restent : re-review Fable en cours, benchmark sans runs ni scores, Cloud Code non enrôlé, aucun provider/K8s, aucun objet copié, T1/T2/T3 non déployés, cutoff du 13 non gelé et facturation non calculée.`,
