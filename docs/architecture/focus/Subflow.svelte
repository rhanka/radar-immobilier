...
<div class="subflow-box" data-node-kind="cluster" data-id={data.entity.id} data-parent-id={data.parentId ?? ''}
  ...
  <header>
    ...
    <div><strong data-text-role="subflow-title">{data.title}</strong>
      <b class="repo-label" data-text-role="repo">{data.provenance.repoLabel}</b></div>
  </header>
</div>
<style>
  .subflow-box { width: 100%; height: 100%; border: 2px solid var(--st-semantic-border-strong); background: var(--st-semantic-surface-subtle); }
  ...
  header div { display: grid; grid-template-columns: auto; gap: 4px 16px; min-width: 0; }
  strong { font-size: 32px; line-height: 1.3; color: var(--st-semantic-text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .repo-label { font-size: 24px; line-height: 1.3; color: var(--st-semantic-text-primary); }
</style>
