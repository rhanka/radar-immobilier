...
const sharedNodes = {
  ...
  PP_REFRESH: immo('cronjob', 'Autonomous Immo refresh', 'Implemented T1 workload · acceptance open', 'radar-immobilier:feat/refresh-018; plan/R018-BRANCH_feat-refresh-018.md'),
...
const targetCommon = {
  ...
  docs_migration: immo('s3', 'Convergence DOCS', 'Pipeline de migration borné et récupérable', 'transitions-target.md:DOCS convergence gate'),
  DOCS_DIFF: immo('check', 'Diff de manifests DOCS', 'Compare keys + hashes PP/PR'),
  DOCS_CANONICAL: immo('check', 'Corpus canonique DOCS', 'Référence initiale exacte = production SCW docs-pocs'),
  DOCS_COPY: immo('release', 'Copie sélective DOCS', 'Tooling gardé · aucun bulk copy du surplus'),
  DOCS_PARITY: immo('check', 'Parité DOCS', '59,017 keys + hashes égaux sur OVH prod/préprod'),
  DOCS_PRUNE: immo('release', 'Retrait MinIO', 'Complet mais seulement après parité + reprise'),
...
const views = {
  ...
  'target-1': { ...targetCommon,
    OVH_CLUSTER: platform('cluster', 'Cluster Kubernetes OVH', 'Plateforme partagée · état effectif de transition'),
    T1_CARD: stage('T1'), T1_CHANGE: stage('T1'), T1_KEEP: stage('T1'), T1_REMOVE: stage('T1'), T1_GATES: stage('T1'), T1_EVIDENCE: stage('T1'),
    PR_OBJECT_GAP: immo('s3', 'Production object migration', 'Audit/migration launched · outcome unknown', 'transitions-target.md:effective transition'),
  },
