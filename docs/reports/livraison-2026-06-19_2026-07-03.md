...
- **Détection : 32 signaux qualifiés sur 30 villes**, émergeant d'un périmètre suivi de **1 106 municipalités** du Québec. La couverture de la chaîne de données atteint **1 006/1 106 municipalités graphées**, **617/1 106 avec zonage servi**, **1 102/1 106 avec lots (cadastre) servis** (bannière live de la vue Sources).
- **Granularité : plusieurs milliers de lots par ville** (5 754 lots à Sainte-Catherine, 10 016 à Mont-Tremblant), chacun scoré (multifamilial 4+, périmètre TOD, potentiel /10) — c'est de cet univers que les ~30 signaux émergent.
...
|---|---|---|
| **Sources — couverture mesurée** : tri-état Servi/Partiel/Non couvert réel (A1), légende unique (A2), colonnes par couche (A3), couverture calculée sur le listing geo live, Console avec toggle Province (1104)/Focus 30, vue Couverture au mode d'interaction Signaux (radio 3 portées, drill zones, scorecard) | #318, #323, #337, #339 | En prod |
...
Vue province : 32 signaux sur 30 villes, filtres par nature de signal à gauche, liste de villes plate, légende par intensité.
...
Couverture par ville en tri-état (Servi / Partiel / Non couvert), radio de portée (Focus QA 4 villes / 30 villes à signaux / province 1 106), et scorecard par ville : état par couche (PV, signaux, zones, normes, lots, TOD), fraîcheur, et prochaine étape :

![Sources — Couverture, scorecard de Sainte-Catherine](assets/sources-couverture-scorecard.png)

Console sources : bannière live (1 006/1 106 graphés · 617/1 106 zonage · 1 102/1 106 lots), périmètre Province (1 104) / Focus 30, filtres par état, et tableau par couche pour chacune des 30 villes à signaux :
...
- **Recomposition PDF → GeoJSON** (levier phare pour le zonage) : chaîne T1 GeoPDF géoréférencé, T2 auto-calage (similarity/Umeyama, arbitrage par lots cadastraux), T3 raster ; 29/30 villes du focus servies par cette chaîne ou une voie directe.
