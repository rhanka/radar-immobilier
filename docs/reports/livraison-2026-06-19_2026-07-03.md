...
**Méthode** : tous les chiffres de ce rapport sont mesurés (journal track, PR GitHub mergées, runs CI, matrice de couverture geo régénérée le 2026-07-03, application en production observée le 3 juillet au soir). Les captures d'écran proviennent de l'application en production (immo.sent-tech.ca), prises les 3 et 4 juillet 2026.
...
**Consolidation données : les 30 villes prioritaires (z∩m∩p) ont toutes leurs PV ET leurs signaux — 0 trou.** Chaque ville du focus (31 villes mesurées sur les graphes S3 du 3 juillet) porte au moins un signal prioritaire et au moins un procès-verbal PDF archivé. Preuve ville par ville : `docs/reports/consolidation-30-2026-07.md`.
...
Fiche lot au clic (zone de rattachement, indicateurs 4+/TOD, potentiel, façade estimée, source et collection geo), avec le filtre de lots (nombre de logements, TOD, priorité, usage, superficie) — ici le lot 3 132 317 à Delson, surligné sur la carte (capture du 4 juillet) :

![Fiche lot et filtres — Delson, lot 3 132 317 sélectionné](assets/fiche-lot-delson.png)

Outil de mesure de distance (nouveau, PR #332), zoomé sur le même lot : le front du lot sur la rue Champlain mesuré à 28 m (la fiche estime la façade à ≈ 19,8 m, sur son autre front) :

![Outil de mesure — façade d'un lot à Delson, 28 m](assets/mesure-facade-delson.png)

### La chaîne de preuve en production

Chaque signal expose sa traçabilité — date d'ingestion, date du document, page, citation exacte, état de chaque élément de preuve (fiche à droite) — et le viewer PDF ouvre le procès-verbal à la page du signal, **citation surlignée en orange** sur la page, avec badge du signal, navigation par signal et lien d'ouverture du document source (capture du 4 juillet, PV de Mont-Tremblant, page 2/3) :

![Viewer PDF de preuve — PV de Mont-Tremblant, citation du signal surlignée en orange](assets/preuve-pdf-mont-tremblant.png)
...
> Détail et table complète : `docs/reports/consolidation-30-2026-07.md`.
...
- **Usage du connecteur MCP** : brancher le connecteur sur claude.ai (guide `docs/guides/connecteur-mcp/`, MD + PDF) et l'utiliser sur les données réelles (signaux, lots, PV, grilles).
- **Extraction des 30 opportunités** : dérouler l'analyse des villes prioritaires via le connecteur (items 2 et 10 du pointage §5).
- **Validation / affinage des critères de ciblage des signaux** : qualifier bons vs mauvais signaux sur les cas concrets (Sainte-Catherine, Cowansville…) pour ajuster les règles de détection (item 8 du pointage §5).

**De notre côté** :

- **Consistance des signaux — PV manquants** : résorber les bris de chaîne de preuve hors focus (14 villes à signaux sans PV archivé, dont brigham déjà résorbée — détail : `docs/reports/consolidation-30-2026-07.md`) et l'incident « Signaux indisponibles » de la vue ville (Sainte-Catherine, §5 action n° 3).
- **Poursuite zonages + normes** : étendre la couverture zonage (658/1 106) et normes (471/1 106, la couche la moins couverte — elle conditionne le scoring de potentiel par lot), et resserrer la propagation acquisition → listing → application (617 vs 658 en zonage).
- **PMTiles (option)** : levier de performance carto pour la montée en charge province entière (0 servi à ce jour, planifié).
- **Planification de features / orientation produit** : prioriser les prochaines fonctionnalités avec Steve à partir de l'usage réel (shopping list/sélection, généralisation TOD, rapport difficultés de scraping — items 4, 5 et 6 du pointage §5).
- **Actualisation des scraps de sources PV** : rafraîchir les sources PV sur le dernier mois et approfondir l'historique des 12 derniers mois.
- **Sources YouTube pour les PV** : initier l'investigation des séances de conseil diffusées sur YouTube comme source complémentaire de PV.
