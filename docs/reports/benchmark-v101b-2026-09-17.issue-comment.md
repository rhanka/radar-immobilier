# Projets de commentaires — v8 finale, non publiés

Liens main : destinations prévues après validation et intégration par le conducteur.

## Variante #697 — benchmark et décision
...
## Variante #703 — rejeu préproduction
...
Graphiques refaits : modèle + effort en police normale sur chaque point ; précision-rappel : couleur = fournisseur, forme = modèle, pointillé = progression d’effort, étoile = cascade ; performance/coût : losange = siège, rond = API ; 28 configurations triées par F1 net, valeurs optimales en gras. Métriques sur 100 toujours non calculables faute d’annotations. Production : https://immo.sent-tech.ca ; préproduction : https://preprod.immo.sent-tech.ca. Aucun commit ni commentaire publié.
