...
## Objective
Correct unsupported UI zoning-family deductions, document the unchanged radar-api boundary and GEO target, and make the Signaux map legends reflect the data actually available.
...
- Do not change radar-api, numeric lot scoring, deployment, or production data.
...
## Feedback Loop
- [x] Owner decisions fixed by the mission: three-tier UI precedence, choice B, no provenance copy in the UI, radar-api unchanged, and `zone_family` owned by GEO.
...
- [x] **Lot 0 — Baseline and evidence**
  ...
  - [x] Trace the UI deduction path, unchanged radar-api boundary, and rendered Checkbox DOM/CSS path.

- [x] **Lot 1 — Correct deduction and provenance**
  - [x] Correct unsupported UI token mappings and add negative regressions.
  - [x] Split conservation and recreation styles and verify rendered legend labels/colors.
  - [x] Record the unresolved radar-api `ID`/`CGS` debt and GEO `zone_family` target.
...
- [x] **Lot 2 — Correct Signaux map legends**
  ...
  - [x] Probe CPTAQ availability at city selection and keep absent agriculture non-interactive.
...
- [x] **Lot 3 — Finalize specification and verification**
  - [x] Write the sourced status table, corrections, residual limits, precedence target, and unchanged radar-api boundary.
  - [x] Run focused UI tests, map suites, `svelte-check`, and branch scope verification.
  - [x] Commit as `docs(spec)` and produce the untracked `.sol-fixup-report.md`.
