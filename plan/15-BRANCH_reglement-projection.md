...
- [ ] **Lot 2 — node creation + statut (D3/D4).** document_type→node (avis/projet→DesignationEvent; adoption→Bylaw; entree_en_vigueur→UPDATE Bylaw; abrogation→state+replaces). statut=RegulatoryStageKind (premier/second→1er/2e-projet PRIMARY deterministic; unknown-doctype→null+flagged; content→null). cible only on avis. Tests: each mapping; unknown→flagged; content→null.
- [ ] **Lot 3 — relation typing (D5, safety-critical).** libellé→replaces(abroge et remplace)·amends(modifiant/modification au) certain; ambiguous→uncertain+flagged; unknown-verb→§9 raw+ignored; target n° from libellé; fromLibelle=verbatim. Tests + NEG: **ambigu-never-auto-amends**; unknown-verb-ignored.
