...
- [x] SCWF-EX5: owner-authorized preprod completion may roll only the proven RAW writer `radar-api` to six explicit OVH Secret references without changing the shared ConfigMap, bind the settled single-Pod observation into a zero-MinIO-writer fence artifact, complete the fenced RAW checkpoint, and consume reviewed proof only for conditional non-destructive copy/reconcile/delta. No scale-to-zero, MinIO/object/PVC/Secret deletion is included; DOCS retirement remains blocked until its distinct live sources and exact OVH destination are complete.
- [x] SCWF-EX6: owner-authorized DOCS preprod completion may add a Make-only operator bundle under `deploy/k8s/object-storage-docs-preprod/`, clone the existing OVH identity into a concern-specific Kubernetes Secret through a silent server-side pipeline, create and head/list-check only the exact BHS bucket `radar-immobilier-docs-preprod`, inventory the complete non-empty MinIO DOCS source plus the proven-empty companion, and perform guarded resumable copy/fence/parity/rebinding for API and refresh workloads. Preserve every source prefix and all recovery evidence; never log Secret data or delete objects, buckets, PVCs, Secrets, or MinIO resources.
- [x] SCWF-EX7: owner-authorized PROD completion may add an isolated Make-only bundle under `deploy/k8s/object-storage-docs-prod/` plus its exact `deploy/ci/` support and root Make include; create a dedicated no-delete OVH identity and Secret for `radar-immobilier-docs`, inventory and non-destructively copy every SCW `radar-immobilier-docs-pocs` object including root-level keys, and prepare independent empty-MinIO retirement. No SCW rebind or deletion precedes parity.
- [x] SCWF-EX8: owner explicitly declares the entire 144,193-object / 28,340,040,620-byte preprod MinIO corpus non-canonical and SCW PROD the sole recovery source, authorizing immediate Make-only deletion of exactly the preprod `radar-minio` StatefulSet, Service, 40Gi PVC and MinIO ingress policies after exact binding/resource proof. Preserve a non-secret before/after receipt; never touch SCW, TEM or either OVH bucket. Rollback can recreate Kubernetes resources but cannot recover the deleted PVC data.
