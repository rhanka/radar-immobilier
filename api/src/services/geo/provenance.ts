 ...
 * rewritten. A truncated envelope (missing a mandatory axis) is rejected, not
 * repaired.
 *
 * No field is a free-form string. Every declared string is validated by
 * POSITIVE FORMAT (contract §7 — enums, ISO-8601 dates, opaque ids, collection
 * slugs, zone codes, reason codes, `sha256:<hex>`, canonical public URL), so a
 * secret, a log line, an S3 key or a path can never ride inside an allowed key.
 * A URL must be HTTP(S) toward a PUBLIC host: a public DNS name, a public IPv4
 * or a global-unicast IPv6 — every loopback / ULA / link-local / IPv4-mapped /
 * multicast / documentation address is refused, in v4 as in v6.
 *
 * `acquisition_v2_readiness.state:"ready"` is only accepted when contract §5's
 * four prerequisites are really carried by the envelope (zone identity, canonical
 * public source, acquisition instant, content sha256) — a `ready` asserted on an
 * empty `unmet_requirement_codes` alone is a false claim and fails the envelope.
 */
...
export function isPublicCanonicalUrl(value: unknown): value is string {
  ...
  if (/[\s\\]/.test(trimmed)) return false; // no embedded whitespace/backslash
 ...
 * s3://bucket/key` can never pass as a "code".
 */
...
function isSafeCodeArray(value: unknown): value is string[] {
  return Array.isArray(value) && value.length <= 64 && value.every(isSafeCode);
}
...
function isNullableSnapshot(value: unknown): value is string | null {
  ...
  return isIsoInstant(value) || isSafeCode(value);
}
...
function validateProofZone(value: unknown): boolean {
  ...
  if (!(method === null || isSafeCode(method))) return false;
  return true;
}
 ...
 * refused as a whole, never silently downgraded (no invention).
 */
function readinessPrerequisitesMet(geometry: unknown): boolean {
  if (!isRecord(geometry)) return false; // no geometry provenance axis at all
...
function validateReadiness(value: unknown, geometry: unknown): boolean {
  ...
  // …and the four real prerequisites.
