...
Status: Updated 2026-05-27 — **evolution track ÉV1–ÉV7 COMPLETE & MERGED** (PR #18–#24) via an autonomous drumbeat. The Phase-1 demo now exposes 10 navigable views: **Onboarding (T0) · Radar · Signaux (T1) · Comparaison · Revue sources · Opportunités (T2) · Grilles de score · Coordination · Console (T3/T4) · Automatisation**, with a global réel/sim toggle and the honest availability-aware scoring (`@radar/scoring`: grids + renormalizing aggregate; 3 real pilots → 3.18/3.35/2.59 partial+capped). Default decisions **D1–D20** + escalations (deferred to a server-side build: real `@sentropic/h2a` adapter, SQL journal/timeline persistence, real job orchestration/scheduler/connectors; + follow-up #31 OpportunityFunnel-legacy-scoreGlobal) in `docs/spec/UAT_EV2_EV7_ESCALATIONS.md` — the agenda for the intermediate UAT. Socle spec PR #17; BR-06/BR-07 materialized by 06V (PR #15). Puis recadrage démo (PR #25/#26) + clôture bugs UAT (PR #27/#28, `docs/spec/UAT_BUG_TRACEABILITY.md` : 38/43 ✅). **UAT round 4** (captures) → intention committée `docs/spec/SPEC_EVOL_UAT_ROUND4.md`. **Next : ÉV12 (UI round 4 : nav horizontale + accordéons Signaux/Sources/Grilles + master-detail clair + grille éditable + libellé filtre lisible) ; amorce ÉV11 (automatiser 1-2 sources réelles + 5ᵉ track benchmark Opus 4.8) ; puis ÉV9 chat, ÉV10 h2a.**
...
- **ÉV7** `feat/automation-benchmark` — **MERGED 2026-05-27 (PR #24).** "Automatisation" view: 3 treatment cadence cards (initial/récurrent/approfondissement) + stub connectors list + honest per-stage agent benchmark recap (reuses comparison data). **Track ÉV1–ÉV7 COMPLETE.** Archived `plan/done/`.
