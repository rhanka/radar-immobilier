...
Status: Updated 2026-05-27 — autonomous drumbeat building the evolution track ÉV1–ÉV7 toward a full UAT. **ÉV1 MERGED (PR #18)** (socle: states + `@radar/scoring` grids/aggregate + calibration 3.18/3.35/2.59 + Grilles view). **ÉV2 MERGED (PR #19)** (Radar T1 signals feed). **ÉV3 MERGED (PR #20)** (Opportunités T2 funnel). **ÉV4 MERGED (PR #21)** (T0 Onboarding/Sources: catalogue checklist + retro-analysis + demo CTA, first nav view). Demo now spans T0→T2 + Grilles. **ÉV5 (h2a spike + chat) is next** — spike-first, decoupled behind an interface; if the external `@sentropic/h2a`/`@sentropic/flow` packages are not demo-usable it falls back to a POLICY+role-label+simple-journal stub and the blocker is logged for UAT. Default decisions + escalations in `docs/spec/UAT_EV2_EV7_ESCALATIONS.md`. Earlier socle spec PR #17; BR-06/BR-07 materialized by 06V (PR #15). Remaining follow-up: persistent journal/timeline SQL (deferred until a writer exists).
...
- **ÉV5** `feat/h2a-spike-chat` — `@sentropic/h2a` spike → V1 POLICY + role label + simple journal (decoupled behind an interface); global chat on `@sentropic/flow` (after its spike). Absorbs BR-08 (graphify) + BR-11 (chat-demo). **Next.**
