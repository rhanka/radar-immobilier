...
# Mont-Tremblant — miroir du harness rimouski (agent-aa3d5326).
set -euo pipefail
cd "$(dirname "$0")"
mkdir -p cites
...
for sha in $SHAS; do
  ...
  {
    echo "Tu es un extracteur de citations pour un procès-verbal municipal (Ville de Mont-Tremblant, Québec)."
...
if not m:
    print(f"[ERR] pas de JSON pour {sha}"); sys.exit(0)
try:
    obj = json.loads(m.group(0))
except Exception as e:
    print(f"[ERR] JSON invalide {sha}: {e}"); sys.exit(0)
