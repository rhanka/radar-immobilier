...
# N_LANES bas (défaut 2) pour protéger la box. Résumable : skip villes déjà citées dans le store de publication.
#
...
#   READ_*    ← identité DOCS dédiée, RO (OVH : raw PVs + .meta.json sidecars)
...
process_city(){
  ...
  if already_cited "$city"; then
    log "[$lane/$city] SKIP déjà cité dans le store PUBLISH"; return 0
  fi
