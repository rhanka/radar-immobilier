...
const campaign = process.env.BENCHMARK_CAMPAIGN ?? "v13";
...
// Both v13 arms share one campaign root, one contract and one output cap, so the bundle
// compares like with like. Aliases are derived from the campaign so a judge cannot map
// them back to a provider by ordering.
const sources = [
  { system: "gemini-low", campaign, root: resolve(campaignRoot, "campaign-gemini") },
  { system: "sonnet-cloudcode", campaign, root: resolve(campaignRoot, "campaign-cloudcode") },
  { system: "sonnet-direct", campaign, root: resolve(campaignRoot, "campaign-direct") },
].map((source) => ({ ...source,
...
for (const document of manifest.documents) {
  ...
  for (const source of sources) {
    ...
    presentSystems.add(source.system);
...
const systemCount = presentSystems.size;
...
const bundleBytes = JSON.stringify({ schemaVersion: 1,
  campaign: `${campaign}${perDocument.some(({ systems }) => systems.length < systemCount) ? "-partial" : ""}`,
  systemCount, coverage: perDocument, judgeInstructions, entries });
...
console.log(JSON.stringify({ entries: entries.length, systemCount,
