...
if (process.env.BENCHMARK_SCORE_OUTPUT) {
  ...
  for (const document of manifest.documents) {
    ...
    const usage = receipt?.actual?.usage;
    cases.push({ caseId: stem, state, attemptCount: receipts.length, receipt: receipt ? { status: receipt.status,
      ...
      actual: receipt.actual && { modelId: receipt.actual.modelId, usage: { inputTokens: usage.inputTokens,
        outputTokens: usage.outputTokens, totalTokens: usage.totalTokens,
        cachedInputTokens: usage.providerRawUsage?.input_tokens_details?.cached_tokens ?? null,
        reasoningTokens: usage.providerRawUsage?.output_tokens_details?.reasoning_tokens ?? null } },
  ...
  const result = { schemaVersion: 1, rankable: false,
    metric: "diagnostic exact frozen anchor + exact stage after v3 provenance gates",
    generatedAt: new Date().toISOString(), protocolDeviation:
      "maxOutputTokens 16384 was requested but absent from every observed wire body", cases };
