...
// normalizeEntityCitations and its bound are module-private, so the offline mirror below
...
if (!profileSource.includes(`MAX_CITATION_EXCERPT_CODE_POINTS = ${MAX_CITATION_EXCERPT_CODE_POINTS}`)
  || !profileSource.includes(`MIN_CITATION_EXCERPT_CHARACTERS = ${MIN_CITATION_EXCERPT_CHARACTERS}`)) {
  throw new Error("Offline citation bounds no longer mirror the profile module");
}
if (REFRESH_PROFILE_CONTRACT_VERSION !== "immo-pv-extraction-v8") {
...
// declared version must match v8 exactly, and the key is stripped before schema validation.
...
// Mirror of validateProvenance under v8: entity citations are no longer rejected for length
// (truncation happened above), only entity-citation and evidence page anchoring remains.
